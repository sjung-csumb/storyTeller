...
# 기본 설정
# ----------------------------------------------------------------------------
BACKEND_URL = os.getenv("BACKEND_URL", "")  # 예: "http://localhost:8001"
...
def generate_story(payload):
    """백엔드가 설정되어 있으면 호출하고, 실패하면 로컬 생성기로 대체합니다."""
    if BACKEND_URL and httpx is not None:
        try:
            resp = httpx.post(f"{BACKEND_URL}/generate", json=payload, timeout=30)
            ...
            st.warning(f"백엔드 연결에 실패해 로컬 샘플 동화로 대체합니다. ({e})")

    return generate_story_local(
        payload["hero_name"], payload["language"], payload["problem"]
    )
